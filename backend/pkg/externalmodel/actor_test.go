...
func TestExternalActor(t *testing.T) {
	...
	expectedActor := externalmodel.Actor{
		...
		ID:                fmt.Sprintf("https://%s/ap/%s", hostURL, acc.PreferredUsername),
		Type:              "Service",
		Following:         fmt.Sprintf("https://%s/ap/%s/following", hostURL, acc.PreferredUsername),
		Followers:         fmt.Sprintf("https://%s/ap/%s/followers", hostURL, acc.PreferredUsername),
		PreferredUsername: acc.PreferredUsername,
		Name:              acc.Name,
		Summary:           acc.Summary,
		Published:         acc.CreatedAt.Format(time.RFC3339),
		Inbox:             fmt.Sprintf("https://%s/ap/%s/inbox", hostURL, acc.PreferredUsername),
		PublicKey: externalmodel.PublicKey{
			ID:           fmt.Sprintf("https://%s/ap/%s#main-key", hostURL, acc.PreferredUsername),
			Owner:        fmt.Sprintf("https://%s/ap/%s", hostURL, acc.PreferredUsername),
